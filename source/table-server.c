...
struct message_t *process_message(struct message_t *msg_pedido,
  ...
  case OC_PUT:
    ...
    msg_resposta->content.result =
        table_put(tabela, msg_pedido->content.entry->key,
  ...
  case OC_UPDATE:
    ...
    msg_resposta->c_type = CT_RESULT;
    msg_resposta->content.result =
        table_update(tabela, msg_pedido->content.entry->key,
                     msg_pedido->content.entry->value);
    if (msg_resposta->content.result == -1) {
...
int main(int argc, char **argv) {
  ...
  poll_list[0].fd =
      listening_socket; // Socket de escuta toma o 1 lugar da lista
